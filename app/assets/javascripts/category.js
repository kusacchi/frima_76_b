  ...
  $(function(){
    ...
    function appendChidrenBox(insertHTML){
      ...
      childSelectHtml = `<select class="exhibitionPage__main__contents__detail__category__choose--select" id="child_category" name="exhibition[category_id]">
                            ...
                            ${insertHTML}
                        <select>`;
      $('.sell__main__content__form__box__group__selects').append(childSelectHtml);
    }
    ...
    function appendGrandchidrenBox(insertHTML){
      ...
      $('.sell__main__content__form__box__group__selects').append(grandchildSelectHtml);
    }
    ...
    $('#parent_category').on('change', function(){
      ...
      if (parent_category_id != "---"){ //親カテゴリーが初期値でないことを確認
        $.ajax({
          url: '/exhibitions/categories/get_category_children',
          type: 'GET',
    ...
    // 子カテゴリー選択後のイベント
    $('.sell__main__content__form__box__group').on('change', '#child_category', function(){
      ...
      if (child_category_id != "---"){ //子カテゴリーが初期値でないことを確認
        $.ajax({
          url: '/exhibitions/categories/get_category_grandchildren',
          type: 'GET',
